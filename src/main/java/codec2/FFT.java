 ...
 * 
 * This code is derived from General Purpose FFT Package written by
 * Takuya Ooura and from JFFTPack written by Baoshe Zhang
 */
package codec2;

/**
 * Computes 1D Discrete Fourier Transform (DFT) of complex single precision
 * floating point data.
 *
 * <p>
 * In-Place algorithm
 *
 * <p>
 * Copyright (C) 2007-2009 Piotr Wendykier, Emory University<br>
 * All Rights Reserved
 */
public final strictfp class FFT {

    private final int n;
    private final float[] wtable;
    private final float[] wtable_r;
    private final int[] factors = {4, 2, 3, 5};
    ...
    /*----------------------------------------------------------------------
     ...
     isign is +1 for backward and -1 for forward transforms
     ----------------------------------------------------------------------*/
    private void passf5(final int ido, final int l1, final float[] in, final int in_off, final float[] out, final int out_off, final int offset, final int isign) /* isign==-1 for forward transform and+1 for backward transform */ {
